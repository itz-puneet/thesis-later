...
\textbf{On how results are reported.} Where an analysis did not answer
the question it was built for, the chapter says so and explains what the
design could not separate, rather than reporting only the comparisons
that worked. One such case arises in \S{}5.3, where an attempt to size the
verification-latency effect is shown to be unidentified and is replaced
by a factorial design that is --- and which then returns an unresolved
result at this sample size. Reporting that is not an aside: it is what
establishes the boundary of what the evidence supports.

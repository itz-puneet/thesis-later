...
Chapter 5 is conditioned on the noise characterised here: the reference
labels defined in \S{}4.1 are the scoring key used throughout it, and the
flip rates estimated in \S{}4.3 are the calibration artefact the later
phases will consume.
...
Every comparison in this report is made against \texttt{label\_oracle},
taken from JIT-Defects4J \citep{ni2022jitdefects4j}, itself an extension
of LLTC4J \citep{herbold2022tangling}. Because the interpretation of
every subsequent result depends on exactly what these labels are, we
state their construction precisely.
...
Three consequences follow, and they constrain the claims this report is
entitled to make.
...
this report --- is \textbf{\texttt{git\ blame} seeded with
...
Six SZZ variants were executed using PySZZ v2
\citep{rosa2023szzvariants}, the reference implementation released with
the developer-informed-oracle study:
...
\begin{figure}
\centering
\includegraphics[width=\linewidth,keepaspectratio]{figures/f1_phase1_noise.png}
...
This asymmetry is the single most consequential fact in this report.
...
problems, and whether they have different downstream consequences is
precisely what the later phases are designed to establish.
...
project}, which is why every statistical test in this report is paired
at the project level rather than pooled over commits, and why any later
noise injection must be calibrated per project rather than globally.

\section{4.3 The bias model}\label{the-bias-model}

The later phases require a parametric description of each variant's
noise, and estimating it is the second deliverable of Phase 1. We adopt
the standard class-conditional formulation, in which a variant is
...
These six pairs are serialised to \texttt{phase1\_bias.json}. They are
the calibration points for the noise-injection profiles Phase 3 will
use: B-SZZ supplies an FP-heavy profile, RA-SZZ an intermediate one, and
L-SZZ an FN-heavy one.
...
unstable as \ensuremath{\rho}\ensuremath{_0} + \ensuremath{\rho}\ensuremath{_1} \ensuremath{\rightarrow} 1, which is a constraint any later mitigation
built on these rates will have to respect.
...
\emph{volume} is not a technicality. It means that a variant with the
higher flip \emph{rate} may none the less contribute fewer wrong labels,
and any account of which SZZ error actually costs a learner will have to
separate the two.
...
than false negatives to whatever consumes the labels. \textbf{That
premise is untested in the literature and is not tested here}; Phase 3
is designed to test it directly, and \S{}6.2 states why the question is not
answerable from Phase 2's evidence alone.

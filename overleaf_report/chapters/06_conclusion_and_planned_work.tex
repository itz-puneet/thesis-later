...
\S{}5.3 shows that the batch-to-streaming drop cannot be decomposed by
comparing label sources, because the contrast is not identified, and
that the 2\ensuremath{\times}2 which \emph{is} identified returns no resolvable effect at
\emph{n} = 21 --- every contrast, including the interaction, has an

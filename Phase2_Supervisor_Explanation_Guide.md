...
**Be careful with the wording.** The measured quantity is the self-scored-minus-oracle-scored gap. It does not by itself demonstrate that the forest "learns which commits BSZZ over-flags" — that is a mechanism the gap is consistent with, not one it establishes. Demonstrating it needs a direct analysis: whether a model trained to predict BSZZ's false positives from the Kamei features achieves above-chance accuracy. **That analysis has now been run, and it supports the mechanism.** Mean AUC **0.599** against a permutation null of **0.514** — HL +0.0876, CI [+0.0405, +0.1312], 17/21 projects, p = 0.0016.

**How to say it.** "SZZ's false positives carry structure the features can pick up, so a high-capacity model can fit that structure instead of fitting defects — we tested it directly rather than inferring it from the gap." Then volunteer the limit before he finds it: **AUC 0.60 is weak separability.** The mechanism is demonstrated to exist; it is not demonstrated to be strong, and the +0.230 gap is almost certainly not explained by this structure alone.

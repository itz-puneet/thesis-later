...
1. ~~`reports/phase1_phase2_report.md` / `.html`~~ — **done.** The generator predated Phase B (no `chronological_online`, no `mcc_avg`, no Holm columns, and it still framed the batch→stream gap as a latency effect), so `scripts/generate_report.py` was retired rather than patched. `Phase1_Phase2_Master_Results.md` is the report now, and `scripts/make_figures.py` reproduces the eight figures from the committed CSVs.
2. ~~`Phase1_Phase2_Comprehensive_Analysis_and_Supervisor_Pitch.md`~~ — **deleted.** Every Phase 2 number in it was stale and it carried the pre-correction framing. Superseded by `Phase1_Phase2_Master_Results.md` (numbers) and `Phase2_Supervisor_Explanation_Guide.md` (presentation). Recoverable from git history if any prose is wanted back.

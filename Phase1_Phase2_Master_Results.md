...
**Mechanism — stated as interpretation, not demonstration.** BSZZ flags 8,060 commits, only 1,495 correctly (18.6% precision). The gap is *compatible with* a high-capacity model fitting systematic structure in BSZZ's errors rather than in defects. It does not by itself establish that; the measured quantity is the self-scored-minus-oracle-scored difference. Demonstrating the mechanism needs a direct test — whether BSZZ's false positives are themselves predictable from the Kamei features above chance. **That test has now been run** (`results/appendix/fp_predictability.csv`): restricted to the 8,060 BSZZ-flagged commits, a Random Forest separates BSZZ's false positives from its true positives at mean AUC **0.599** against a label-permutation null of **0.514** — paired HL +0.0876, CI [+0.0405, +0.1312], 17/21 projects, p = 0.0016. The mechanism is real: SZZ's errors carry learnable structure. It is also **weak** — AUC 0.60 is modest separability — so the wording should be "the error structure is learnable", not "SZZ's mistakes are easy to predict".

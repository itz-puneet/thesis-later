...
The chapter is deliberately placed before any predictive modelling. Chapter 5 is conditioned on the noise characterised here: the reference labels defined in §4.1 are the scoring key used throughout it, and the flip rates estimated in §4.3 are the calibration artefact the later phases will consume.
...
This asymmetry is the single most consequential fact in this report. **Modelling SZZ noise as symmetric random label flips — the standard assumption in the learning-with-noisy-labels literature — misrepresents it entirely.** A variant that misses seven defects in ten and one that raises four false alarms for every true one are different problems, and whether they have different downstream consequences is precisely what the later phases are designed to establish.
...
B-SZZ's recall ranges from 0.220 to 0.919 across projects — a span of seven-tenths. AG-SZZ's ranges from 0.074 to 0.819. **A single corpus-level noise rate is a poor description of any individual project**, which is why every statistical test in this report is paired at the project level rather than pooled over commits, and why any later noise injection must be calibrated per project rather than globally.

---

## 4.3 The bias model

The later phases require a parametric description of each variant's noise, and estimating it is the second deliverable of Phase 1. We adopt the standard class-conditional formulation, in which a variant is treated as a noisy channel applied to the reference labels:
...
These six pairs are serialised to `phase1_bias.json`. They are the calibration points for the noise-injection profiles Phase 3 will use: B-SZZ supplies an FP-heavy profile, RA-SZZ an intermediate one, and L-SZZ an FN-heavy one.

**Two properties of this channel are worth stating explicitly.**

First, ρ₀ + ρ₁ approaches 0.8 for the conservative variants. Loss-correction methods that invert the noise channel become numerically unstable as ρ₀ + ρ₁ → 1, which is a constraint any later mitigation built on these rates will have to respect.

Second, because the positive class is only 8.54% of the corpus, **equal flip rates do not imply equal numbers of flipped labels.** B-SZZ's ρ₀ of 0.263 applied to 24,987 clean commits produces 6,565 false positives; L-SZZ's ρ₁ of 0.733 applied to 2,332 defective commits produces 1,710 false negatives. This asymmetry between *rate* and *volume* is not a technicality. It means that a variant with the higher flip *rate* may none the less contribute fewer wrong labels, and any account of which SZZ error actually costs a learner will have to separate the two.
...
That relocation is defensible only if false positives are more costly than false negatives to whatever consumes the labels. **That premise is untested in the literature and is not tested here**; Phase 3 is designed to test it directly, and §6.2 states why the question is not answerable from Phase 2's evidence alone.

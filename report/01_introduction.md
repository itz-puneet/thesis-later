...
Four things must be separated to make progress: the noise in the labels; the inflation contributed by the evaluation protocol; the *mechanism* by which noise damages an online learner; and whether a learner can defend itself.

**This report covers the first two.** Phase 1 characterises the noise; Phase 2 measures what it costs downstream and separates that cost from the inflation contributed by the evaluation protocol. The remaining two are stated as research questions, their designs are given in Chapter 6, and **no result is claimed for either.**
...
**RQ3.** Through what mechanism does label noise degrade an online learner — specifically, how does ORB's oversampling response behave under symmetric versus SZZ-calibrated asymmetric noise as the dose increases? *(Not addressed in this report; design in §6.2)*

**RQ4.** Can an online learner recover performance lost to SZZ-origin noise without access to clean labels, and without sacrificing performance when the labels are clean? *(Not addressed in this report; design in §6.2)*
...
**A calibrated noise model released as an artefact.** The class-conditional flip rates (ρ₀, ρ₁) for all six variants, estimated over a common denominator and serialised for reuse, so that downstream noise studies can calibrate against measured SZZ behaviour rather than assume symmetric noise.
...
**Chapters 4 and 5** report Phases 1 and 2, answering RQ1 and RQ2 respectively.

**Chapter 6** concludes: it synthesises the two results, gives the full treatment of validity threats, and sets out the design of the remaining phases together with what this report establishes that they now depend on.

**On how results are reported.** Where an analysis did not answer the question it was built for, the chapter says so and explains what the design could not separate, rather than reporting only the comparisons that worked. One such case arises in §5.3, where an attempt to size the verification-latency effect is shown to be unidentified and is replaced by a factorial design that is — and which then returns an unresolved result at this sample size. Reporting that is not an aside: it is what establishes the boundary of what the evidence supports.
